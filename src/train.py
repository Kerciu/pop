...
def main():
    ...
    episodes = 5000  # Możesz dać więcej
    epsilon = 1.0
    epsilon_decay = 0.999
    epsilon_min = 0.05
    best_score = -float("inf")

    print("--- START TRENINGU (Fast Mode + Sorting) ---")
    ...
    for e in range(episodes):
        ...
        while not done:
            ...
            # 2. W BAZIE I PUSTY BAK -> TANKUJ
            elif dist_to_base <= problem.D and current_state_obj.carrot_count < 20:
                forced_action = 5

            # 3. U CELU -> DOSTARCZ (Zawsze sprawdzamy loaded_gifts[0], bo jest posortowane)
